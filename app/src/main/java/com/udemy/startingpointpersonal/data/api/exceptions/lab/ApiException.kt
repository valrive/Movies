package com.udemy.startingpointpersonal.data.api.exceptions.lab
...
sealed class ApiException<out T> {
    ...
    data class HttpCode<out T>(val data: T) : ApiException<T>()

    /**
     * Error occurred when request could not be processed because session has expired
     * @param message error message
     */
    data class SessionExpired<out T>(val data: T) : ApiException<T>()
